...
namespace QueueServiceTests
{
    public class QueueProcessing
    {
        ...
        public async void RequestTimesOutIfNotHandled()
        {
            ...
            var result  = await _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request);
        ...
        {
            var request           = new TestQueuedRequest { image_name = "Bob.jpg" };
            var firstrequestTask  = _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request);
            var secondRequestTask = _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request);
        ...
        public async Task TimeoutRequestDoesntClogQueue()
        {
            ...
            var request1Task = _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request1);

            await Task.Delay(queueOptions.ResponseTimeout + TimeSpan.FromSeconds(5));
            var request2Task = _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request2);
        ...
        {
            var tasks = new List<Task<BackendResponseBase>>();
        ...
        public async Task RequestCanBeCanceled()
        {
            ...
            var request     = new TestQueuedRequest { image_name = "Bob.jpg" };
            var requestTask = _queueServices.SendRequestAsync<TestQueuedResponse>(QueueName, request, cts.Token);
            cts.Cancel();
