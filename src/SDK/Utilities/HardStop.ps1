...
$appDir = Join-Path -Path $pwd -ChildPath "..\.."
...
if ($Debug)
{
    ...
    get-process | where {$_.CommandLine -Like "$appDir\modules\*"} | kill
}
else
{
    ...
    {
        Write-Host "Deleting CodeProject.AI Server downloads..." -ForegroundColor DarkGreen
        Remove-Item "$appDir\downloads\*" -Recurse
    }
    ...
    {
        Write-Host "Deleting CodeProject.AI Server Modules..." -ForegroundColor DarkGreen
        Remove-Item "$appDir\modules\*" -Recurse
    }
