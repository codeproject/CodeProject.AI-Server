...
class PipelinedModelRunner:
  ...
  def __init__(self, interpreters):
    ...
    # TODO: If the interpreter is CPU not TPU based then this call will fail.
    # While this class is designed for TPU, there are cases where the TPU is
    # recognised but we can't actually create an interpreter for it. We could
    # bail and go back to CPU only code, or we could modify this class to provide
    # a mock-runner that can take the push/pop and perform the inference directly
    # using the provided CPU interpreter. That allows us to seamlessly use this 
    # code even in the even of a device fail.
    # WARN: This will fail for interpreters created for CPU
