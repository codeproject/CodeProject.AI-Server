...
def make_interpreter(model_file: str, cpu_model_file: str = None, num_of_threads: int = 1) -> Tuple[any, bool]:
    """ make tf-lite interpreter.

    If model_file is provided, but no cpu_model_file, then we assume the caller has
    determined the libraries and hardware that is available and has supplied a
    suitable file. Otherwise, this method will assume the model file is an edgetpu 
    model but will sniff libraries and hardware and fallback to cpu_model_file if 
    edge TPU support isn't available.
    ...
    cpu_model_file = cpu_model_file or model_file
    ...
    # On Windows, the interpreter.__init__ method accepts experimental delegates.
    # These are used in self._interpreter.ModifyGraphWithDelegate, which fails on Windows
    try:
        import platform
    ...
    if delegates:
        ...
        interpreter = Interpreter(model_path=model_file, experimental_delegates=delegates)
        edge_tpu = True
    else:
