...
namespace CodeProject.AI.Analysis.Yolo
{
    ...
    public class YoloProcessor : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken token)
        {
            ...
            _logger.LogTrace("Background YoloDetector Task Started.");
            await _codeprojectAI.LogToServer("CodeProject.AI Object Detection module started.",
                                             "YoloProcessor", LogLevel.Information,
                                             string.Empty, token);
        ...
        private async Task ProcessQueue(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (file is null)
                {
                    await _codeprojectAI.LogToServer("No File supplied for object detection.", 
                                                     "YoloProcessor", LogLevel.Error, string.Empty,
                                                     token);
                ...
                else
                {
                    ...
                    {
                        await _codeprojectAI.LogToServer($"Object Detection Error for {file.filename}.",
                                                         "YoloProcessor", LogLevel.Error,
                                                         string.Empty, token);
        ...
        {
            _logger.LogTrace("Background YoloDetector Task is stopping.");
