        ...
        private List<YoloPrediction> ParseDetect(DenseTensor<float> output, SKImage image)
        {
            ...
            try
            {
                ...
                Parallel.For(0, numBoxes, (i) =>
                {
                    ...
                    if (boxConfidence <= _model.Confidence) return; // skip low obj_conf results
                    ...
                    xMin = Math.Clamp(xMin, 0, w - 1); // clip bbox tlx to boundaries
                    yMin = Math.Clamp(yMin, 0, h - 1); // clip bbox tly to boundaries
                    xMax = Math.Clamp(xMax, 0, w - 1); // clip bbox brx to boundaries
                    yMax = Math.Clamp(yMax, 0, h - 1); // clip bbox bry to boundaries

                    var requiredConfidence = _model.Confidence / boxConfidence;
                    for (int k = 5; k < boxInfoLength; k++)
                    {
                        if (buffer[k] <= requiredConfidence) continue; // skip low mul_conf results

                        YoloLabel label = _model.Labels[k - 5];

                        var prediction = new YoloPrediction(label, buffer[k] * boxConfidence)
                        {
            ...
            try
            {
                ...
                using var inferenceResults = Inference(resized ?? image);
                {
                    if (inferenceResults != null)
                    {
                        ...
                        List<YoloPrediction> predictions = ParseOutput(output.ToArray(), image);
