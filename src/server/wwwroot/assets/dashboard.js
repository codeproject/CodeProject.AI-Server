...
async function checkForUpdates(showResult) {
    ...
    await makeGET('server/status/updateavailable', function (data) {
...
async function getSystemStatus() {
    ...
    await makeGET('server/status/system-status', 
        function (result) {
...
async function getModulesStatuses() {
    ...
    await makeGET('server/status/analysis/list?random=' + new Date().getTime(), 
        function (data) {
                ...
                for (let i = 0; i < data.statuses.length; i++) {
                    ...
                    if (!row) {
                        ...
                        if (status == "NotAvailable") {
                            ...
                            rowHtml += 
                                   `<div class='proc-count text-end text-nowrap me-1' title='Requests processed' style='width:3rem'>${processedCount}</div>`
...
async function getMeshStatus() {
    await makeGET('server/mesh/summary', 
        function (result) {
...
async function getLogs() {
    ...
    await makeGET('log/list?count=' + logLinesPerRequest + '&last_id=' + _lastLogId, 
        function (data) {
...
async function getDownloadableModules() {
    ...
    await makeGET('module/list/?random=' + new Date().getTime(), 
        function (data) {
            if (data && data.modules) {
                ...
                for (let i = 0; i < data.modules.length; i++) {
                    ...
                    let author          = moduleInfo.author     || '';
                    let homepage        = moduleInfo.homepage   || '';
                    let basedOn         = moduleInfo.basedOn    || '';
                    let basedOnUrl      = moduleInfo.basedOnUrl || '';
